function model = Get_Sens_ODE(model,salt,parameter)
...
sys_ODEs = @(t,x,k,d) tmp_ODE_AA(t,x,k,d)+salt(t)*tmp_ODE_BB(t,x,k,d);
model.sys_ODEs = sys_ODEs;
% sens ODEs uses all x. 
sens_ODEs = @(t,x,k,d) model.Jacobian(t,x(1:n_nodes),k,d)*x(n_nodes+1:end)+ ...
    K_matrix_A(t,x(1:n_nodes),k,d)+salt(t)*K_matrix_B(t,x(1:n_nodes),k,d);
